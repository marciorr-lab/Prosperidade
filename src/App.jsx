...
const CATEGORIES = [
{ id:“mercado”,     label:“Mercado”,     emoji:“🛒”, color:”#4ade80” },
{ id:“alimentacao”, label:“Alimentacao”, emoji:“🍽️”, color:”#fb923c” },
{ id:“educacao”,    label:“Educacao”,    emoji:“📚”, color:”#60a5fa” },
{ id:“saude”,       label:“Saude”,       emoji:“🏥”, color:”#f472b6” },
{ id:“transporte”,  label:“Transporte”,  emoji:“🚗”, color:”#a78bfa” },
{ id:“lazer”,       label:“Lazer”,       emoji:“🎉”, color:”#fbbf24” },
{ id:“roupas”,      label:“Roupas”,      emoji:“👕”, color:”#34d399” },
{ id:“casa”,        label:“Casa”,        emoji:“🏠”, color:”#f87171” },
{ id:“assinaturas”, label:“Assinaturas”, emoji:“📱”, color:”#38bdf8” },
{ id:“outros”,      label:“Outros”,      emoji:“📦”, color:”#94a3b8” },
];

const INCOME_TYPES = [
{ id:“salario”,   label:“Salario”,   emoji:“💼”, color:”#4ade80” },
{ id:“freelance”, label:“Freelance”, emoji:“🛠️”, color:”#60a5fa” },
{ id:“reembolso”, label:“Reembolso”, emoji:“↩️”, color:”#fbbf24” },
{ id:“bonus”,     label:“Bonus”,     emoji:“🏆”, color:”#fb923c” },
{ id:“aluguel”,   label:“Aluguel”,   emoji:“🏘️”, color:”#a78bfa” },
{ id:“dividendo”, label:“Dividendo”, emoji:“📈”, color:”#34d399” },
{ id:“presente”,  label:“Presente”,  emoji:“🎁”, color:”#f472b6” },
{ id:“outro”,     label:“Outro”,     emoji:“📦”, color:”#94a3b8” },
];

const PAYMENT_METHODS = [
{ id:“corrente”, label:“Conta Corrente”, emoji:“🏦” },
{ id:“card1”,    label:“Cartao 1”,       emoji:“💳” },
{ id:“card2”,    label:“Cartao 2”,       emoji:“💳” },
{ id:“card3”,    label:“Cartao 3”,       emoji:“💳” },
];

const PERSONS = [
{ id:“p1”, emoji:“👤”, color:”#7c6af7” },
{ id:“p2”, emoji:“👤”, color:”#f472b6” },
];
...
<div>
<div style={{fontSize:10, color:pos?”#4ade80”:”#f87171”, letterSpacing:1}}>SALDO DO MES</div>
...
<div>Saidas: <span style={{color:”#f87171”}}>{fmt(expense)}</span></div>
</div>
</div>
);
}
...
<div style={{color:”#6b6b8a”, fontSize:13}}>Sincronizando…</div>
...
const [names,     setNames]     = useState({ p1:“Voce”, p2:“Parceiro(a)” });
...
setInvest({ accounts:data.filter(d => d.type === “account”), contributions:data.filter(d => d.type === “contribution”) });
done();
});
...
<div style={{fontSize:18, fontWeight:“bold”, color:”#f0ede8”}}>Prosperidade</div>
<Spinner/>
</div>
);

return (
<div style={{minHeight:“100vh”, background:”#0f0f13”, color:”#f0ede8”, fontFamily:“Georgia,serif”, maxWidth:430, margin:“0 auto”, paddingBottom:90}}>

```
  {syncing && <div style={{position:"fixed", top:0, left:0, right:0, zIndex:100, background:"#7c6af7", padding:"6px", textAlign:"center", fontSize:11, color:"#fff", letterSpacing:1}}>Sincronizando...</div>}

  <div style={{background:"linear-gradient(135deg,#1a1a24,#12121a)", borderBottom:"1px solid #2a2a3a", padding:(syncing ? "22px" : "16px") + " 16px 12px", position:"sticky", top:0, zIndex:10}}>
    <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start"}}>
      <div>
        <div style={{fontSize:10, color:"#6b6b8a", letterSpacing:3}}>FAMILIA</div>
        <div style={{fontSize:19, fontWeight:"bold", letterSpacing:-0.5}}>Prosperidade</div>
      </div>
    ...
    <div style={{display:"flex", gap:6, marginTop:10, alignItems:"center", flexWrap:"wrap"}}>
      ...
      <button onClick={() => setEditNames(v => !v)} style={{background:"transparent", border:"1px solid #2a2a3a", borderRadius:20, padding:"4px 8px", fontSize:10, color:"#6b6b8a", cursor:"pointer", fontFamily:"inherit"}}>editar</button>
    </div>

    {editNames && (
      <div style={{display:"flex", gap:8, marginTop:8}}>
        {PERSONS.map(p => (
          <input key={p.id} value={names[p.id]} onChange={e => setNames(prev => ({ ...prev, [p.id]:e.target.value }))} style={{...IS, padding:"7px 10px", fontSize:12, flex:1}} placeholder="Nome"/>
        ))}
    ...
    <div style={{display:"flex", gap:5, marginTop:10, overflowX:"auto", paddingBottom:2}}>
      {MONTHS.map((m, i) => (
        <button key={i} onClick={() => setSelMonth(i)} style={{background:selMonth === i ? "#7c6af7" : "#1e1e2e", color:selMonth === i ? "#fff" : "#6b6b8a", border:"none", borderRadius:20, padding:"5px 11px", fontSize:11, cursor:"pointer", whiteSpace:"nowrap", fontFamily:"inherit", fontWeight:selMonth === i ? "bold" : "normal"}}>{m}</button>
      ))}
    </div>
  </div>

  <div style={{display:"flex", borderBottom:"1px solid #1e1e2e", background:"#12121a"}}>
    {[["add","+ Lancar"],["dashboard","Dash"],["history","Lista"],["compare","Meses"],["invest","Invest"]].map(([id, lbl]) => (
      <button key={id} onClick={() => setTab(id)} style={{flex:1, padding:"12px 2px", border:"none", background:"transparent", color:tab === id ? "#7c6af7" : "#4a4a6a", fontSize:10, cursor:"pointer", fontFamily:"inherit", borderBottom:tab === id ? "2px solid #7c6af7" : "2px solid transparent", fontWeight:tab === id ? "bold" : "normal"}}>{lbl}</button>
    ))}
  </div>
  ...
  <div style={{padding:"16px 14px"}}>
    ...
    {tab === "add" && (
      <div>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", background:"#1a1a24", borderRadius:12, padding:3, marginBottom:16, border:"1px solid #2a2a3a"}}>
          {[["expense","Gasto","#f87171"],["income","Entrada","#4ade80"]].map(([mode, lbl, col]) => (
            <button key={mode} onClick={() => setEntryMode(mode)} style={{background:entryMode === mode ? col + "22" : "transparent", border:entryMode === mode ? "1.5px solid " + col : "1.5px solid transparent", borderRadius:10, padding:"10px", color:entryMode === mode ? col : "#6b6b8a", fontSize:14, fontWeight:"bold", cursor:"pointer", fontFamily:"inherit"}}>{lbl}</button>
          ))}
        </div>
        ...
        {entryMode === "expense" && (
          <div style={{display:"flex", flexDirection:"column", gap:12}}>
            <input placeholder="Descricao (ex: Supermercado)" value={ef.description} onChange={e => setEf(f => ({ ...f, description:e.target.value }))} style={IS}/>
            <div style={{display:"flex", gap:10}}>
              <input placeholder="Valor" type="number" min="0" value={ef.amount} onChange={e => setEf(f => ({ ...f, amount:e.target.value }))} style={{...IS, flex:1}}/>
              <input type="date" value={ef.date} onChange={e => setEf(f => ({ ...f, date:e.target.value }))} style={{...IS, flex:1}}/>
            </div>
            <div>
              <div style={{fontSize:11, color:"#6b6b8a", marginBottom:8}}>QUEM GASTOU</div>
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
                {PERSONS.map(p => (
                  <button key={p.id} onClick={() => setEf(f => ({ ...f, person:p.id }))} style={{background:ef.person === p.id ? p.color + "33" : "#1e1e2e", border:ef.person === p.id ? "1.5px solid " + p.color : "1.5px solid #2a2a3a", borderRadius:10, padding:"11px 8px", color:ef.person === p.id ? p.color : "#6b6b8a", fontSize:13, cursor:"pointer", fontFamily:"inherit", fontWeight:ef.person === p.id ? "bold" : "normal"}}>{names[p.id]}</button>
                ))}
              </div>
            </div>
            <div>
              <div style={{fontSize:11, color:"#6b6b8a", marginBottom:8}}>CATEGORIA</div>
              <div style={{display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:6}}>
                {CATEGORIES.map(c => (
                  <button key={c.id} onClick={() => setEf(f => ({ ...f, category:c.id }))} style={{background:ef.category === c.id ? c.color + "33" : "#1e1e2e", border:ef.category === c.id ? "1.5px solid " + c.color : "1.5px solid #2a2a3a", borderRadius:10, padding:"8px 4px", fontSize:20, cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:2}}>
                    <span>{c.emoji}</span>
                    <span style={{fontSize:7, color:ef.category === c.id ? c.color : "#4a4a6a"}}>{c.label}</span>
                  </button>
                ))}
              </div>
            </div>
            <div>
              <div style={{fontSize:11, color:"#6b6b8a", marginBottom:8}}>PAGAMENTO</div>
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
                {PAYMENT_METHODS.map(p => (
                  <button key={p.id} onClick={() => setEf(f => ({ ...f, paymentMethod:p.id }))} style={{background:ef.paymentMethod === p.id ? "#7c6af733" : "#1e1e2e", border:ef.paymentMethod === p.id ? "1.5px solid #7c6af7" : "1.5px solid #2a2a3a", borderRadius:10, padding:"10px 8px", color:ef.paymentMethod === p.id ? "#c4b9ff" : "#6b6b8a", fontSize:12, cursor:"pointer", fontFamily:"inherit", textAlign:"left"}}>{p.emoji} {p.label}</button>
                ))}
              </div>
              {ef.paymentMethod !== "corrente" && (
                <div style={{background:"#1e1a2e", border:"1px solid #3a2a6a", borderRadius:8, padding:"8px 12px", marginTop:8, fontSize:12, color:"#a78bfa"}}>
                  Fatura de {MONTHS[(new Date(ef.date + "T12:00:00").getMonth() + 1) % 12]} (venc. dia 10)
                </div>
              )}
            </div>
            <button onClick={addExpense} style={{background:"linear-gradient(135deg,#c0392b,#e74c3c)", border:"none", borderRadius:12, padding:"14px", color:"#fff", fontSize:16, fontWeight:"bold", cursor:"pointer", fontFamily:"inherit", marginTop:4}}>Lancar Gasto</button>
          </div>
        )}

        {entryMode === "income" && (
          <div style={{display:"flex", flexDirection:"column", gap:12}}>
            <input placeholder="Descricao (ex: Salario Maio)" value={inf.description} onChange={e => setInf(f => ({ ...f, description:e.target.value }))} style={IS}/>
            <div style={{display:"flex", gap:10}}>
              <input placeholder="Valor" type="number" min="0" value={inf.amount} onChange={e => setInf(f => ({ ...f, amount:e.target.value }))} style={{...IS, flex:1}}/>
              <input type="date" value={inf.date} onChange={e => setInf(f => ({ ...f, date:e.target.value }))} style={{...IS, flex:1}}/>
            </div>
            <div>
              <div style={{fontSize:11, color:"#6b6b8a", marginBottom:8}}>QUEM RECEBEU</div>
              <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8}}>
                {PERSONS.map(p => (
                  <button key={p.id} onClick={() => setInf(f => ({ ...f, person:p.id }))} style={{background:inf.person === p.id ? p.color + "33" : "#1e1e2e", border:inf.person === p.id ? "1.5px solid " + p.color : "1.5px solid #2a2a3a", borderRadius:10, padding:"11px 8px", color:inf.person === p.id ? p.color : "#6b6b8a", fontSize:13, cursor:"pointer", fontFamily:"inherit", fontWeight:inf.person === p.id ? "bold" : "normal"}}>{names[p.id]}</button>
                ))}
              </div>
            </div>
            <div>
              <div style={{fontSize:11, color:"#6b6b8a", marginBottom:8}}>TIPO DE ENTRADA</div>
              <div style={{display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:6}}>
                {INCOME_TYPES.map(t => (
                  <button key={t.id} onClick={() => setInf(f => ({ ...f, type:t.id }))} style={{background:inf.type === t.id ? t.color + "33" : "#1e1e2e", border:inf.type === t.id ? "1.5px solid " + t.color : "1.5px solid #2a2a3a", borderRadius:10, padding:"8px 4px", fontSize:20, cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", gap:2}}>
                    <span>{t.emoji}</span>
                    <span style={{fontSize:7, color:inf.type === t.id ? t.color : "#4a4a6a"}}>{t.label}</span>
                  </button>
                ))}
              </div>
            </div>
            <button onClick={addIncome} style={{background:"linear-gradient(135deg,#16a34a,#22c55e)", border:"none", borderRadius:12, padding:"14px", color:"#fff", fontSize:16, fontWeight:"bold", cursor:"pointer", fontFamily:"inherit", marginTop:4}}>Registrar Entrada</button>
          </div>
        )}
      </div>
    )}
    ...
    {tab === "dashboard" && (
      <div>
        ...
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11, color:"#6b6b8a", marginBottom:8}}>POR PESSOA</div>
          {PERSONS.map(p => {
            ...
            return (
              <div key={p.id} style={{background:"#1a1a24", borderRadius:12, padding:"12px 14px", marginBottom:8, borderLeft:"3px solid " + p.color}}>
                <div style={{display:"flex", justifyContent:"space-between"}}>
                  <span style={{fontWeight:"bold", color:p.color}}>{names[p.id]}</span>
                  <span style={{fontWeight:"bold", color:bal >= 0 ? "#4ade80" : "#f87171"}}>{fmt(bal)}</span>
                </div>
                <div style={{display:"flex", gap:16, marginTop:6, fontSize:11, color:"#6b6b8a"}}>
                  <span>Entrou: <span style={{color:"#4ade80"}}>{fmt(inc)}</span></span>
                  <span>Saiu: <span style={{color:"#f87171"}}>{fmt(exp)}</span></span>
                </div>
              </div>
            );
          })}
        ...
        {byType.length > 0 && (
          <div style={{marginBottom:14}}>
            <div style={{fontSize:11, color:"#6b6b8a", marginBottom:8}}>ENTRADAS POR TIPO</div>
            {byType.map(t => (
              <div key={t.id} style={{marginBottom:10}}>
                ...
                <div style={{background:"#1e1e2e", borderRadius:99, height:6, overflow:"hidden"}}>
                  <div style={{background:t.color, height:"100%", width:((t.total / maxType) * 100) + "%", borderRadius:99}}/>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14}}>
          <div style={CS("#4ade8022","#4ade80")}>
            <div style={{fontSize:10, color:"#4ade80"}}>CONTA CORRENTE</div>
          ...
          <div style={CS("#7c6af722","#7c6af7")}>
            <div style={{fontSize:10, color:"#a78bfa"}}>CARTOES (fatura)</div>
            <div style={{fontSize:17, fontWeight:"bold", marginTop:4}}>{fmt(byCard.filter(p => p.id !== "corrente").reduce((s, p) => s + p.total, 0))}</div>
          </div>
        </div>
        <div>
          <div style={{fontSize:11, color:"#6b6b8a", marginBottom:10}}>GASTOS POR CATEGORIA</div>
          {byCat.length === 0 && <div style={{color:"#4a4a6a", textAlign:"center", padding:16}}>Sem gastos neste mes</div>}
          {byCat.map(c => (
            <div key={c.id} style={{marginBottom:12}}>
              ...
              <div style={{background:"#1e1e2e", borderRadius:99, height:6, overflow:"hidden"}}>
                <div style={{background:c.color, height:"100%", width:((c.total / maxCat) * 100) + "%", borderRadius:99}}/>
              </div>
    ...
    {tab === "history" && (
      <div>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", background:"#1a1a24", borderRadius:10, padding:3, marginBottom:14, border:"1px solid #2a2a3a"}}>
          {[["expense","Gastos"],["income","Entradas"]].map(([mode, lbl]) => (
            <button key={mode} onClick={() => setEntryMode(mode)} style={{background:entryMode === mode ? "#2a2a3a" : "transparent", border:"none", borderRadius:8, padding:"9px", color:entryMode === mode ? "#f0ede8" : "#6b6b8a", fontSize:13, cursor:"pointer", fontFamily:"inherit"}}>{lbl}</button>
          ))}
        </div>
        ...
        {entryMode === "expense" && (
          <>
            <div style={{display:"flex", gap:6, marginBottom:10, flexWrap:"wrap"}}>
              ...
              <select value={fCat} onChange={e => setFCat(e.target.value)} style={SS}><option value="all">Categorias</option>{CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}</select>
              <select value={fPm}  onChange={e => setFPm(e.target.value)}  style={SS}><option value="all">Pagamento</option>{PAYMENT_METHODS.map(p => <option key={p.id} value={p.id}>{p.label}</option>)}</select>
            </div>
            <div style={{fontSize:11, color:"#6b6b8a", marginBottom:10}}>{filteredExp.length} lancamentos — {fmt(filteredExp.reduce((s, e) => s + e.amount, 0))}</div>
            {filteredExp.length === 0 && <div style={{color:"#4a4a6a", textAlign:"center", padding:30}}>Nenhum gasto encontrado</div>}
            {filteredExp.map(e => {
              ...
              return (
                <div key={e.id} style={{background:"#1a1a24", borderRadius:12, padding:"12px 14px", marginBottom:8, borderLeft:"3px solid " + (cat?.color || "#4a4a6a"), display:"flex", alignItems:"center", gap:12}}>
                  <div style={{flex:1, minWidth:0}}>
                    <div style={{marginBottom:4}}><span style={{fontSize:10, background:(person?.color || "#4a4a6a") + "33", color:person?.color, borderRadius:10, padding:"2px 8px"}}>{names[e.person] || e.person}</span></div>
                    <div style={{fontSize:14, fontWeight:"bold", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>{e.description}</div>
                    <div style={{fontSize:11, color:"#6b6b8a", marginTop:2}}>{cat?.emoji} {cat?.label} — {pm?.label} — {(e.date || "").split("-").reverse().join("/")}</div>
                  ...
                  <div style={{display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4}}>
                    ...
                    <button onClick={() => delExpense(e.id)} style={{background:"transparent", border:"none", color:"#4a4a6a", fontSize:16, cursor:"pointer", padding:0}}>🗑</button>
                  </div>
                </div>
              );
            })}
        ...
        {entryMode === "income" && (
          <>
            <div style={{display:"flex", gap:6, marginBottom:10, flexWrap:"wrap"}}>
              <select value={fPer}  onChange={e => setFPer(e.target.value)}  style={SS}><option value="all">Todos</option>{PERSONS.map(p => <option key={p.id} value={p.id}>{names[p.id]}</option>)}</select>
              <select value={fType} onChange={e => setFType(e.target.value)} style={SS}><option value="all">Tipos</option>{INCOME_TYPES.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}</select>
            </div>
            ...
            {filteredInc.length === 0 && <div style={{color:"#4a4a6a", textAlign:"center", padding:30}}>Nenhuma entrada encontrada</div>}
            {filteredInc.map(i => {
              ...
              return (
                <div key={i.id} style={{background:"#1a1a24", borderRadius:12, padding:"12px 14px", marginBottom:8, borderLeft:"3px solid " + (type?.color || "#4ade80"), display:"flex", alignItems:"center", gap:12}}>
                  <div style={{flex:1, minWidth:0}}>
                    <div style={{marginBottom:4}}><span style={{fontSize:10, background:(person?.color || "#4a4a6a") + "33", color:person?.color, borderRadius:10, padding:"2px 8px"}}>{names[i.person] || i.person}</span></div>
                    <div style={{fontSize:14, fontWeight:"bold", whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis"}}>{i.description}</div>
                    <div style={{fontSize:11, color:"#6b6b8a", marginTop:2}}>{type?.emoji} {type?.label} — {(i.date || "").split("-").reverse().join("/")}</div>
                  </div>
                  <div style={{display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4}}>
                    <span style={{fontSize:15, fontWeight:"bold", color:type?.color || "#4ade80"}}>+{fmt(i.amount)}</span>
                    <button onClick={() => delIncome(i.id)} style={{background:"transparent", border:"none", color:"#4a4a6a", fontSize:16, cursor:"pointer", padding:0}}>🗑</button>
                  </div>
                </div>
              );
            })}
    ...
    {tab === "compare" && (
      <div>
        <div style={{fontSize:11, color:"#6b6b8a", marginBottom:14}}>COMPARATIVO MENSAL</div>
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14}}>
          {[{ lbl:MONTHS[prevM], inc:prevInc.reduce((s,i) => s+i.amount,0), exp:prevT }, { lbl:MONTHS[selMonth], inc:totalInc, exp:totalExp }].map((col, idx) => {
            ...
            return (
              <div key={idx} style={CS(idx === 1 ? "#7c6af722" : "#1e1e2e", idx === 1 ? "#7c6af7" : "#4a4a6a")}>
                <div style={{fontSize:11, color:idx === 1 ? "#a78bfa" : "#6b6b8a"}}>{col.lbl.toUpperCase()}</div>
                ...
                <div style={{fontSize:10, color:"#6b6b8a", marginTop:4, lineHeight:1.6}}>
                  <div>Entrou: {fmt(col.inc)}</div>
                  <div>Saiu: {fmt(col.exp)}</div>
                </div>
              </div>
            );
          })}
        ...
        {diffP && (
          <div style={{background:diff > 0 ? "#2d0f0f" : "#0f2d1a", border:"1px solid " + (diff > 0 ? "#f87171" : "#4ade80"), borderRadius:12, padding:"12px 16px", marginBottom:16, display:"flex", alignItems:"center", gap:10}}>
            ...
            <div>
              ...
              <div style={{fontSize:12, color:"#6b6b8a"}}>vs. {MONTHS[prevM]} ({diff > 0 ? "+" : "-"}{Math.abs(diffP)}%)</div>
            </div>
          </div>
        )}
        <div>
          <div style={{fontSize:11, color:"#6b6b8a", marginBottom:12}}>POR CATEGORIA</div>
          {CATEGORIES.map(c => {
            const curr = byCat.find(b => b.id === c.id)?.total || 0;
            const prev = prevBC[c.id] || 0;
            if (curr === 0 && prev === 0) return null;
            const cd = curr - prev;
            const mb = Math.max(curr, prev, 1);
            return (
    ...
    {tab === "invest" && (
      <div>
        <div style={{fontSize:11, color:"#6b6b8a", marginBottom:14}}>INVESTIMENTOS</div>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16}}>
          <div style={CS("#fbbf2422","#fbbf24")}>
            <div style={{fontSize:10, color:"#fbbf24"}}>PATRIMONIO</div>
          ...
          <div style={CS("#34d39922","#34d399")}>
            <div style={{fontSize:10, color:"#34d399"}}>TOTAL APORTADO</div>
            <div style={{fontSize:18, fontWeight:"bold", marginTop:4}}>{fmt(totContr)}</div>
          </div>
        </div>
        <div style={{marginBottom:16}}>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10}}>
            <div style={{fontSize:11, color:"#6b6b8a"}}>CONTAS</div>
          ...
          {showAcc && (
            <div style={{background:"#1a1a24", border:"1px solid #2a2a3a", borderRadius:12, padding:14, marginBottom:12, display:"flex", flexDirection:"column", gap:8}}>
              ...
              <input placeholder="Instituicao (ex: Rico)" value={newAcc.institution} onChange={e => setNewAcc(f => ({ ...f, institution:e.target.value }))} style={IS}/>
              <input placeholder="Saldo atual" type="number" value={newAcc.balance} onChange={e => setNewAcc(f => ({ ...f, balance:e.target.value }))} style={IS}/>
              ...
              <div key={acc.id} style={{background:"#1a1a24", borderRadius:12, padding:"14px", marginBottom:10, borderLeft:"3px solid " + acc.color}}>
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start"}}>
                  ...
                  <div style={{textAlign:"right", display:"flex", flexDirection:"column", alignItems:"flex-end", gap:4}}>
                    ...
                    <div style={{fontSize:10, color:"#6b6b8a"}}>{pct}% do portfolio</div>
                    <button onClick={() => delInvestment(acc.id)} style={{background:"transparent", border:"none", color:"#4a4a6a", fontSize:14, cursor:"pointer", padding:0}}>🗑</button>
                  </div>
                </div>
          ...
          {invest.accounts.length === 0 && <div style={{color:"#4a4a6a", textAlign:"center", padding:20, fontSize:13}}>Nenhuma conta cadastrada</div>}
        </div>
        <div style={{marginBottom:20}}>
          <div style={{fontSize:11, color:"#6b6b8a", marginBottom:10}}>REGISTRAR APORTE</div>
          <div style={{display:"flex", flexDirection:"column", gap:8}}>
            ...
            <div style={{display:"flex", gap:8}}>
              <input placeholder="Valor" type="number" value={invForm.amount} onChange={e => setInvForm(f => ({ ...f, amount:e.target.value }))} style={{...IS, flex:1}}/>
              <select value={invForm.month} onChange={e => setInvForm(f => ({ ...f, month:e.target.value }))} style={SS}>
                {MONTHS.map((m, i) => <option key={i} value={i}>{m}</option>)}
              </select>
            </div>
            <button onClick={addContrib} style={{background:"linear-gradient(135deg,#34d399,#059669)", border:"none", borderRadius:12, padding:"13px", color:"#fff", fontSize:15, fontWeight:"bold", cursor:"pointer", fontFamily:"inherit"}}>Registrar Aporte</button>
          </div>
        </div>
        {contribByM.length > 0 && (
          <div>
            <div style={{fontSize:11, color:"#6b6b8a", marginBottom:12}}>APORTES POR MES</div>
            {contribByM.map((c, i) => (
              <div key={i} style={{marginBottom:12}}>
                ...
                <div style={{background:"#1e1e2e", borderRadius:99, height:8, overflow:"hidden"}}>
                  <div style={{background:"linear-gradient(90deg,#34d399,#059669)", height:"100%", width:((c.total / maxC) * 100) + "%", borderRadius:99}}/>
                </div>
              </div>
            ))}
          </div>
